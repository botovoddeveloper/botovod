...
void c_grin::Terminate()
{
	str x = "Application Terminate.\n\n\n"
			...
			"В случае неисправности обращайтесь к разработчику:\n\n"
			"-------------------------------------------------------------\n\n"
...
void c_grin::DevLog( bool enabled, str data )
{
	if ( enabled )
	{
		...
		try
		{
			if ( ! DirectoryExists( dir.t_str() ) ) CreateDirectory( dir.t_str(), NULL );
...
str  c_vk::GetParameter( str ParameterName, str *ResponseData, bool Cut, bool ifInteger, bool *Received )
{
	...
	str data;
	if ( ifInteger ) data = tw+ParameterName+tw+":";
	else             data = tw+ParameterName+tw+":"+tw;
...
str  c_vk::GetParameterIntString( int index, str Parameter1, str Parameter2, str *Response1, str *Response2, str Data )
{
	...
	for ( int c = 1; c <= index; c++ )
	{
		...
		if ( c == index )
		{
			...
			if ( p1 > 0 ) { buff = ActualData.SubString(1,p1-1); istext = true; }
			*Response1 = buff;
			if ( istext )
			{
...
str  c_vk::users_get( bool *success, str UserIdsCommaLine, str Token, bool MySelf )
{
	...
	try
	{
		...
		str           URL;
		if ( MySelf ) URL = "https://api.vk.com/method/users.get?access_token="+Token+"&v="+vk.API_VERSION;
		else          URL = "https://api.vk.com/method/users.get?access_token="+Token+"&v="+vk.API_VERSION+"&user_ids="+UserIdsCommaLine;
...
str  c_vk::users_search( bool *success, str Request, str Token, int OffSet, int Count, int AgeFrom, int AgeTo, bool OnlyOnline, bool HasPhoto, str Interests )
{
	...
	try
	{
		...
		str Online  = "&online=0"; if ( OnlyOnline ) Online = "&online=1";
		str exPhoto = "&has_photo=0"; if ( HasPhoto ) exPhoto = "&has_photo=1";
		str exInterests = ""; if ( Interests.Length() != 0 ) exInterests = "&interests="+Interests;
...
str  c_vk::friends_add( bool *success, str UserID, str Token, str Message, str CaptchaSID, str CaptchaANS )
{
	...
	try
	{
		...
		if ( CaptchaSID.Length() > 0 ) CaptchaSID = "&captcha_sid=" + CaptchaSID;
		if ( CaptchaANS.Length() > 0 ) CaptchaANS = "&captcha_key=" + CaptchaANS;
...
void c_static_advanced::connect( str Client_ID, str login, str pass, str *token )
{
	...
	INI->WriteString( "OAUTH2", "client_id", Client_ID  );
	INI->WriteString( "OAUTH2", "login",     login 		);
	INI->WriteString( "OAUTH2", "password",  pass 		);
